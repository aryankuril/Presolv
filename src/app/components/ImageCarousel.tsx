...
interface ImageCarouselProps {
  ...
  autoplay?: boolean;
  scrollDelay?: number; // Delay in milliseconds
  slidesPerView?: {
    mobile?: number;
    tablet?: number;
    desktop?: number;
...
const ImageCarousel = ({ 
  ...
  showPagination = true,
  autoplay = true,
  scrollDelay = 3000, // Default 3 seconds
  /* eslint-disable @typescript-eslint/no-unused-vars */
  slidesPerView = { mobile: 2, tablet: 3, desktop: 4 },
  slidesToScroll = { mobile: 2, tablet: 3, desktop: 1 },
  ...
  return (
    <div className={`container relative w-full overflow-hidden ${className}`}>
                    ...
                    {item.title && (
                      item.link ? (
                        <Link href={item.link} target="_blank" rel="noopener noreferrer">
                          <div className={`hover:text-blue-400 transition-colors ${titleClassName || ''}`}>
                            {item.title}
                          </div>
                        </Link>
                      ) : (
